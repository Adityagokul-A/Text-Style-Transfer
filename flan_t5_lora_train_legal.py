...
# Ensure you have a file named "legal2.json" in the same directory
try:
    with open("legal2.json", "r", encoding="utf-8") as f:
        data = json.load(f)  # list of dicts with "legal_sentence" and "casual_sentence"
        print(f"Loaded {len(data)} examples from legal2.json")
except FileNotFoundError:
    print("Error: legal2.json not found. Please create this file with your dataset.")
    # Creating a dummy dataset for demonstration if the file doesn't exist
    data = [
        {"legal_sentence": "The plaintiff's legal counsel is seeking to depose all key witnesses prior to the commencement of the trial.", "casual_sentence": "The lawyer for the person suing wants to interview all the main witnesses before the trial starts."},
        {"legal_sentence": "This document constitutes a binding agreement, and any breach thereof shall result in severe financial penalties.", "casual_sentence": "This is a serious agreement, and if you break it, you'll have to pay a big fine."}
    ]
...
def preprocess_function(examples):
    ...
    labels = tokenizer(
        text_target=examples["casual_sentence"],
        max_length=128,
        truncation=True,
        padding="max_length"
    )
...
output_dir = "./flan-t5-base-lora-legal-style-transfer"
